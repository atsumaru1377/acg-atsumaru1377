...
class HelloWorld(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        (
            ...
            self.vtx2bones,
            self.vtx2weights,
            self.bone2parentbone,
            self.bone2invBindingMatrix,
            self.channels,
        ) = parse_gltf.load_data(
        ...
        edge2vtx = util_for_task08.edge2vtx_from_tri2vtx(
            tri2vtx
        ...
        ebo = self.ctx.buffer(
            edge2vtx
        )  # send triangle index data to GPU (element buffer object)
        vbo_ini = self.ctx.buffer(
            self.vtx2xyz_ini
        )  # send initial vertex coordinates data to GPU
        self.vao_ini = self.ctx.vertex_array(
            self.prog, [(vbo_ini, "3f", "in_vert")], ebo, 4, mode=moderngl.LINES
        )  # tell gpu about the mesh information and how to draw it
        self.vbo_def = self.ctx.buffer(
            self.vtx2xyz_def
        )  # send deformed vertex coordinates data to GPU
        self.vao_def = self.ctx.vertex_array(
            self.prog, [(self.vbo_def, "3f", "in_vert")], ebo, 4, mode=moderngl.LINES
        )  # tell gpu about the mesh information and how to draw it
        ...
        vbo_cyl = self.ctx.buffer(
            vtx2xyz_cyl
        )  # send deformed vertex coordinates data to GPU
        self.vao_cyl = self.ctx.vertex_array(
            self.prog, [(vbo_cyl, "3f", "in_vert")], ebo, 4, mode=moderngl.TRIANGLES
        )
    ...
    def render(self, time, frame_time):
        ...
        bone2relativeTransformation = parse_gltf.get_relative_transformations(
            time, num_bone, self.channels
        )
        ...
        for i_vtx in range(self.vtx2xyz_ini.shape[0]):  # for each point in mesh
            ...
            p0 = np.append(
                p0, np.array([1.0])
            )  # homogeneous coordinate of undeformed point
            p1 = np.array(
                [0.0, 0.0, 0.0, 1.0], dtype=np.float32
            )  # p1 is the deformed point
            for idx in range(4):  # in gltf each vertex is associated with four bones
                ...
                inverseBindingMatrix = self.bone2invBindingMatrix[
                    i_bone
                ]  # 4x4 inverse binding matrix
                globalTransformation = bone2globalTransformation[
                    i_bone
                ]  # 4x4 transformation matrix to deformed bone
                ...
                p1 += w * np.matmul(
                    globalTransformation, np.matmul(inverseBindingMatrix, p0)
                )

            self.vtx2xyz_def[i_vtx] = p1[
                :3
            ]  # from homogeneous coordinates to the Cartesian coordinates

        self.vbo_def.write(self.vtx2xyz_def)
        self.ctx.clear(1.0, 1.0, 1.0)

        # view transformation for undeformed character
        transform_to_center = pyrr.Matrix44.from_translation((-0.8, 0.0, -0.8))
        ...
        transform_to_center = pyrr.Matrix44.from_translation((+0.6, 0.0, -0.8))
        view_rot_x = pyrr.Matrix44.from_x_rotation(np.pi * 0.5)
        view_rot_y = pyrr.Matrix44.from_y_rotation(np.pi * 0.3)
        view_transf = view_rot_y * view_rot_x * transform_to_center
        ...
        for i_bone in range(num_bone):
            ...
            self.prog["color"].value = (0.0, 0.0, 0.8)
            self.vao_cyl.render()
            # x_axis
            x_rot = pyrr.Matrix44.from_y_rotation(math.pi * 0.5)
            self.prog["matrix"].value = tuple(
                (np.matmul(x_rot.transpose(), transf)).flatten()
            )  # column major
            self.prog["color"].value = (0.8, 0.0, 0.0)
            self.vao_cyl.render()
            # y_axis
            y_rot = pyrr.Matrix44.from_x_rotation(-math.pi * 0.5)
            self.prog["matrix"].value = tuple(
                (np.matmul(y_rot.transpose(), transf)).flatten()
            )  # column major
            self.prog["color"].value = (0.0, 0.8, 0.0)
            self.vao_cyl.render()
