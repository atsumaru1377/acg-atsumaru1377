...
class HelloWorld(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.tri2vtx, self.vtx2xyz_ini = (
            util_for_task09.load_triangle_mesh_from_wavefront_obj_file(asset_path)
        )
        ...
        self.fixback2vtx = (
            (
                np.logical_and(
                    self.vtx2xyz_ini[:, 0] > +0.15, self.vtx2xyz_ini[:, 2] > +0.115
                )
            )
            .nonzero()[0]
            .tolist()
        ...
        coeff_penalty = 100.0
        fix2vtx = numpy.concatenate(
            [self.fixback2vtx, self.fixear2vtx, self.fixbase2vtx]
        )
    ...
    def render(self, time, frame_time):
        ...
        self.vtx2xyz_def[self.fixback2vtx, 2] += -0.3 * math.sin(2.0 * time)
        ...
        A = D + L
        for i in range(3):
            b = D @ self.vtx2xyz_def[:, i] + L @ self.vtx2xyz_ini[:, i]
            self.vtx2xyz_def[:, i] = spsolve(A, b)

        ## Problem 3
        L2 = self.matrix_bilaplace
        A = D + L2
        for i in range(3):
            b = D @ self.vtx2xyz_def[:, i] + L2 @ self.vtx2xyz_ini[:, i]
            self.vtx2xyz_def[:, i] = spsolve(A, b)
        ...
        view_z_flip = pyrr.Matrix44.from_scale((1.3, 1.3, -1.3, 1.0))
        ...
        for i_vtx in self.fixbase2vtx:
            ...
            model_transf = pyrr.Matrix44.from_translation(
                pos
            ) * pyrr.Matrix44.from_scale((r, r, r, 1.0))
